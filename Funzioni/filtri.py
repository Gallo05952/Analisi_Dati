...
class FiltroTempo:
    ...
    def FineTransitorio(self, tempo):
        tempo = datetime.strptime(tempo,'%Y-%m-%d %H:%M:%S').time()  # Convert the string to a datetime.time object
        start_time = datetime.strptime(self.time[0], '%Y-%m-%d %H:%M:%S').time()  
        ...
        self.df.iloc[:, 0] = pd.to_datetime(self.df.iloc[:, 0], format='%Y-%m-%d %H:%M:%S').dt.time
    ...
    def FiltraPerTempo(self, start_time_str, end_time_str):
        ...
        self.df.iloc[:, 0] = pd.to_datetime(self.df.iloc[:, 0], format='%Y-%m-%d %H:%M:%S')
        ...
        df_filtered.iloc[:, 0] = pd.to_datetime(df_filtered.iloc[:, 0]).dt.strftime('%Y-%m-%d %H:%M:%S')
